...
'''
# input 
- 'packets in hex format

# part I
    
- parse the packets
- convert hex into binary string
- first 3 bits: vertion
- next 3 bits: type
- type=4: literal packets
    - 5 bit chunks, first bit is a 'continuation
        (when 1: more chunks; when 0, last chunk.)
    - next bit: 'mode'
        mode = 0:
            next 15 bits is a bit-length of sub-packets
        mode = 1:
            next 11 bits is a number of sub-packets
            
- sums up all the  `version`
    
# part II

the `type` has meaning:

- 0: sum all sub-packets
- 1: multiply all sub-packets
- 2: minimum of all sub-packets
- 3: maximum of all sub-packets
- 4: literal packet from above
- 5: packet[0] > packet[1]
- 6: packet[0] < packet[1]
- 7: packet[0] == packet[1]
'''
...
data = open('./data_16.in').read().strip()
